...
#import "DDBaseMJTableView.h"
#import "DDRefreshTextHeader.h"

@interface DDBaseMJTableView()

@property (nonatomic,assign)DDTableRefreshType refreshHeaderType;

@end

@implementation DDBaseMJTableView
...
-(void)_initRefreshView {
    ...
    //1.初始化header
    [self setupRefreshHeaderType:DDTableRefreshTypeGIF];
}
...
/**
 设置生效header
 */
-(void)setupRefreshHeaderType:(DDTableRefreshType)refreshHeaderType{
    self.refreshHeaderType = refreshHeaderType;
    switch (refreshHeaderType) {
        case DDTableRefreshTypeNormal:{
            [self stateHeaderNormal];
            break;
        }
        case DDTableRefreshTypeGIF:{
            [self stateHeaderGIF];
            break;
        }
        default:
...
-(void)headerRereshing{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        @strongify(self);
        if([self.refreshDelegate respondsToSelector:@selector(pullDown:tableView:)]){
            self.currentPageNo = 1;
            [self.refreshDelegate pullDown:self.currentPageNo tableView:self];
        }
    });
}
...
-(void)footerRereshing{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        @strongify(self);
        if([self.refreshDelegate respondsToSelector:@selector(pullUp:tableView:)]){
            self.currentPageNo += 1;
            [self.refreshDelegate pullUp:self.currentPageNo tableView:self];
